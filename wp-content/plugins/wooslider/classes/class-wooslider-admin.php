...
class WooSlider_Admin {
	...
	public function admin_styles_global () {
		global $wooslider;
		wp_register_style( $wooslider->token . '-global', $wooslider->plugin_url . 'assets/css/global.css', '', '1.0.0', 'screen' );
...
<form action="media-new.php" method="post" id="wooslider-insert">
	...
	<div id="wooslider-advanced-settings">
		...
		<?php settings_fields( $wooslider->settings->token ); ?>
		<?php do_settings_sections( $wooslider->settings->token ); ?>
...
<?php
	...
	public function media_tab_js () {
		...
		wp_register_script( $wooslider->token . '-media-tab', esc_url( $wooslider->plugin_url . 'assets/js/shortcode-creator.js' ), '1.0.0', '', true );
...
<?php
	...
	private function generate_conditional_fields_slides () {
		...
		$fields['slide_page'] = array( 'name' => __( 'Slide Pages', 'wooslider' ), 'type' => 'multicheck', 'args' => $categories_args, 'description' => __( 'The slide pages from which to display slides', 'wooslider' ) );
