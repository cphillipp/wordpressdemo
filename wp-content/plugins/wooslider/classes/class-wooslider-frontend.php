...
class WooSlider_Frontend {
	...
	private function generate_single_slider_javascript ( $id, $args, $extra = array() ) {
		...
		$html .= "\n" . 'jQuery( \'.wooslider-id-' . esc_attr( intval( $id ) ) . '\' ).flexslider2(' . $args_output . ');' . "\n";
	...
	private function generate_slider_settings_javascript ( $args, $extra = array() ) {
		...
		if ( isset( $extra['thumbnails'] ) && ( $extra['thumbnails'] == 'true' ) ) {
	...
	public function enqueue_scripts () {
		...
		wp_register_script( $this->token . '-sliders', home_url( '/?' . $this->token . '-javascript=load' ), array( $this->token . '-flexslider' ), '1.0.0', true );
